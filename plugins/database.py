...
def _insert_to_db(
    file_list: list, table: str, schema: str = "public", season_id: int = None
):
    ...
    with engine.connect() as conn:
        ...
        if season_id:
            conn.execute(
                f'DELETE FROM {schema}.{table} WHERE "SEASON_ID" = {season_id};'
            )
        else:
...
def insert_game_data_to_db(**kwargs):
    ...
    for pt in ["hitter", "pitcher", "fielder", "runner"]:
        summaries = glob.glob(
            f"output/processed/player/{year}/{pt}/season_summary.parquet"
        )
        ...
        situations = glob.glob(
            f"output/processed/player/{year}/{pt}/*/situation.parquet"
        )
